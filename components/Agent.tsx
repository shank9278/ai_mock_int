...
const Agent = ({userName, userId, type}: AgentProps) => {
    ...
    const handelCall = async () => {
    alert("Voice call feature is not enabled yet. Complete VAPI setup first.");
};

const handelDisconnect = async () => {
    alert("Call end — but actually VAPI was never started.");
};
        ...
        {messages.length > 0 && (
            <div className="transcript-border">
                <div className="transcript">
                  <p key={latestMessage} className={cn('transition0opacity duration-500 opacity-0', 'animate-fadeIn opacity-100')} >
                    {latestMessage}
                   </p>  
                </div>
            </div>
